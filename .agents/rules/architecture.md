...
- **Profile and write-policy locks have one order.** A flow that needs both
  acquires the profile lock before the write-policy lock. Reversing that order
  can deadlock concurrent login, policy, and mutation commands.
