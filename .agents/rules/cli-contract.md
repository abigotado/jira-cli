...
`assets/skills/jira/reference/contract.md` are generated from `internal/errx`
...
| 6 | retryable (rate limit, read network, timeout, or local lock) | follow the hint, then retry safely |
| 7 | confirmation required | add `--yes` |
| 8 | permission / scope denied | stop and request permission or scope |
| 9 | conflict / stale state | re-read the Jira object before retrying |
...
- Every future mutating command accepts `--dry-run` and `--yes`, registered by
  the shared command helper rather than per command.
...
**not** a passthrough of Jira's response: the client decodes into its own
