...
- **HTTP**: `net/http/httptest`. Never call the real Jira API from a test.
- **Keychain**: inject a fake `CredentialStore`. Ordinary tests never touch the
  real OS keychain. The opt-in cross-artifact spike may create, read, and delete
  only its own unique non-secret sentinel through Security.framework.
- **Filesystem**: `t.TempDir()`. Never write to the developer's real cache
  directory.
- **Environment**: `t.Setenv()`. Never mutate `os.Environ` directly.

A test that depends on the developer's home directory, network, or Jira
...
- Classic and scoped token base URLs stay on their fixed Atlassian hosts.
- Oversized and non-JSON upstream bodies never escape through errors.
- 429 with and without `Retry-After`.
- A non-JSON error body. Jira returns 401 as `text/plain`; a client that
